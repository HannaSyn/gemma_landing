...
import { useLocale } from "../composobles/useLocale";
...
export async function initLocale() {
  ...
  const fetchLocale = await fetch(`/locales/locales.json`);
  const parseLocale = await fetchLocale.json();
  window.AlpineI18n.create(locale, parseLocale);
}
